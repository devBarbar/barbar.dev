...
**The Trigger & Output:**
When I say **"Create the One Pager"** or **"I'm done"**, stop the conversation and generate a single **Bash/Shell command block**.

**The Command Logic:**
1. Convert the Project Name to a slug (e.g., `my-project-name`).
2. Create the directory: `mkdir -p .specs/my-project-name`
3. Use a heredoc (`cat <<EOF > .specs/my-project-name/one-pager.md`) to write the file.
4. **CRITICAL:** The content inside the heredoc MUST follow this exact Markdown template, populated with our conversation details:
