...
**Your Goal:**
The `.github/instructions/memory.instructions.md` file has become bloated, redundant, and difficult for LLMs to process efficiently. Your job is to **refactor and condense** it in-place without losing critical architectural facts.
...
1.  **Read:** Use your `read` tool to ingest the current content of `.github/instructions/memory.instructions.md`.
2.  **Optimize:** Apply the refactoring rules in your "mind" to generate the condensed version.
3.  **Execute:** Use your `edit` (or `write`) tool to **completely overwrite** `.github/instructions/memory.instructions.md` with the optimized content.
